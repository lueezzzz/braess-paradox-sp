...
candidate_links = [(u, v, d['name']) for u, v, d in G.edges(data=True) if d['flow'] > 1.0]
print(f"\n--- Iteratively Testing {len(candidate_links)} Active Links ---")

results = []
for index, (u, v, road_name) in enumerate(candidate_links):
    print(f"\n{index+1}/{len(candidate_links)}: Removing {road_name} ({u} -> {v})...", end="\t")
    
    # Save edge data and temporarily remove it
    edge_data = G[u][v].copy()
    G.remove_edge(u, v)
    
    # Test the altered network
    new_tstt, dropped_vol = solve_equilibrium(G, routing_demands, max_iter=30) 
    
    # Restore the edge immediately
    G.add_edge(u, v, **edge_data)
    
    # Analyze Results
    if dropped_vol > 0:
        print("Cut-edge detected! Demand dropped. Skipping analysis.", end="\t")    
    ...
    is_paradox = time_difference_mins < -0.5 # Setting a threshold of half a minute to avoid floating point noise
...
print("\n\n--- TOP 10 FINDINGS ---")
df_results = pd.DataFrame(results).sort_values(by='Change (Minutes)', ascending=True)

# Print the top 15 links that reduced TSTT the most when deleted
