...
def writeResults(network: FlowTransportNetwork, output_file: str, costFunction=BPRcostFunction,
    ...
    # Clearly label the top of the file
    mode_label = "SYSTEM OPTIMAL (SO)" if systemOptimal else "USER EQUILIBRIUM (UE)"
    outFile.write(f"ASSIGNMENT TYPE:\t{mode_label}\n")
...
def load_network(net_file: str, demand_file: str = None, verbose: bool = True, force_net_reprocess: bool = False) -> FlowTransportNetwork:
    """
    Modified loader to handle the new Python-based data format.
    """
    readStart = time.time()

    if verbose:
        print(f"Loading network from Python file: {net_file}...")

    # Redirect to the importer in network_import.py
    # Note: We rename net_file_py to net_file to match the computeAssignment call
    net_df, demand_df = import_network_from_py(net_file)
...
if __name__ == '__main__':
    ...
    net_file = "sioux_falls_data.py"

    # --- SYSTEM OPTIMAL (SO) RUN ---
    print("\n" + "="*30)
    print("RUNNING SYSTEM OPTIMAL (SO)")
    print("="*30)
    
    total_system_travel_time_optimal = computeAssignment(
        net_file=net_file,
        algorithm="FW",
        costFunction=BPRcostFunction,
        systemOptimal=True,
        ...
        accuracy=0.00001,
        maxIter=1000,
        results_file="sioux_falls_SO_flow.tntp"  # Explicit SO filename
    )

    # --- USER EQUILIBRIUM (UE) RUN ---
    print("\n" + "="*30)
    print("RUNNING USER EQUILIBRIUM (UE)")
    print("="*30)
    
    total_system_travel_time_equilibrium = computeAssignment(
        net_file=net_file,
        algorithm="FW",
        costFunction=BPRcostFunction,
        systemOptimal=False,
        ...
        accuracy=0.001,
        maxIter=1000,
        results_file="sioux_falls_UE_flow.tntp"  # Explicit UE filename
    )

    # --- FINAL TERMINAL COMPARISON ---
    print("\n" + "!"*30)
    print("FINAL COMPARISON RESULTS")
    print(f"SO Total Travel Time: {total_system_travel_time_optimal:.2f}")
    print(f"UE Total Travel Time: {total_system_travel_time_equilibrium:.2f}")
    print(f"Difference (UE - SO): {total_system_travel_time_equilibrium - total_system_travel_time_optimal:.2f}")
    print("!"*30)
